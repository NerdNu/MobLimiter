...
public class MobLimiter extends JavaPlugin implements Listener {
    ...
    public void onCreatureSpawnEvent(final CreatureSpawnEvent e) {
        if (e.getSpawnReason() == SpawnReason.BREEDING && isFarmAnimal(e.getEntity())) {
    ...
    public void removeMobs(Chunk c) {
        net.minecraft.server.v1_5_R1.Chunk chunk = ((CraftChunk) c).getHandle();
        Map<String, Integer> count = new HashMap<String, Integer>();
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < chunk.entitySlices[i].size(); j++) {
                Object obj = chunk.entitySlices[i].get(j);
                Entity entity = null;
                if (obj instanceof net.minecraft.server.v1_5_R1.Entity) {
                    entity = ((net.minecraft.server.v1_5_R1.Entity) obj).getBukkitEntity();
                } else {
                    entity = (Entity) obj;
                }
                if (entity != null) {
